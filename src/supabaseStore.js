...
function batchSlotEventId(slotKey) {
  const bytes = createHash('sha256')
    .update(`parts-seals:auto-batch:${slotKey}`)
    .digest()
    .subarray(0, 16);
...
export async function claimBatchSlot(slotKey, claimedAt = new Date()) {
  const claimedIso = new Date(claimedAt).toISOString();
  const { error } = await getSupabaseAdmin()
    .from('events')
    .insert({
      id: batchSlotEventId(slotKey),
      type: 'email.batch.claimed',
      at: claimedIso,
      details: { slot: slotKey },
    });
  if (error?.code === '23505') return false;
  if (error) throw new Error(`Reserva do lote automático: ${error.message}`);
  return true;
}
